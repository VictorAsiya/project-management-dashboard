...
const client = new Client()
  .setEndpoint(process.env.REACT_APP_APPWRITE_ENDPOINT || "https://YOUR_APPWRITE_ENDPOINT")
  .setProject(process.env.REACT_APP_APPWRITE_PROJECT_ID || "YOUR_PROJECT_ID");

const databases = new Databases(client);
const DATABASE_ID = process.env.REACT_APP_DATABASE_ID || "YOUR_DATABASE_ID";
const COLLECTION_ID = process.env.REACT_APP_COLLECTION_ID || "invoices";
...
export default function FinanceDashboard() {
  ...
  const togglePaid = async (id, currentStatus) => {
    try {
      ...
      const doc = await databases.updateDocument(DATABASE_ID, COLLECTION_ID, id, updated);
      setInvoices((prev) =>
        prev.map((i) => (i.$id === id ? doc : i))
      );
    } catch (err) {
      console.error(err);
